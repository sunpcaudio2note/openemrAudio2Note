...
class OpenEMRAudioSoapNoteManager
{
    private $pid;
    private $encounterId;
    ...
    /**
     * Saves or updates a structured SOAP note in OpenEMR's form_soap_audio table.
     *
     * @param array $transcriptionResults An associative array containing structured transcription data (subjective, objective, assessment, plan, transcript, remarks).
     * @return mixed The ID of the saved/updated form_soap_audio entry on success, false on failure.
     */
    public function saveNoteData(array $transcriptionResults)
    {
        ...
        $assessment = ''; // The assessment content is now part of the plan.
        $plan = $transcriptionResults['billing_content'] ?? '';
        $transcript = $transcriptionResults['raw_transcript'] ?? '';
        $remarks = $transcriptionResults['remarks'] ?? '';

        if (empty($subjective) && empty($objective) && empty($assessment) && empty($plan)) {
            error_log("OpenEMRAudioSoapNoteManager WARNING: All mapped SOAP fields are empty for form_audio_to_note_id: " . $this->form_audio_to_note_id . ". Check backend output structure.");
        }

        if (empty($this->pid) || empty($this->encounterId) || empty($this->userId)) {
            error_log("OpenEMRAudioSoapNoteManager CRITICAL: PID, EncounterID, or UserID is missing for form_audio_to_note_id: " . $this->form_audio_to_note_id);
            return false;
        }
        ...
        if (!$audioNoteMetaData || empty($audioNoteMetaData['linked_forms_id'])) {
            error_log("OpenEMRAudioSoapNoteManager CRITICAL: Could not retrieve metadata or linked_forms_id for form_audio_to_note ID {$this->form_audio_to_note_id}.");
            return false;
        }
        ...
        $existingSoapData = sqlQuery("SELECT id FROM form_soap_audio WHERE id = ?", [$targetFormsId]);

        if ($existingSoapData && isset($existingSoapData['id'])) {
            $updateSql = "UPDATE form_soap_audio SET
                            date = ?, user = ?, groupname = ?, authorized = ?, activity = ?,
                            subjective = ?, objective = ?, assessment = ?, plan = ?, transcript = ?, remarks = ?, pid = ?
                          WHERE id = ?";
            $bindings = [
                $currentDateTime, $user, $groupname, $authorized, $activity,
                $subjective, $objective, $assessment, $plan, $transcript, $remarks, $this->pid,
                $targetFormsId
            ];
            if (sqlStatement($updateSql, $bindings) === false) {
                error_log("OpenEMRAudioSoapNoteManager CRITICAL: Failed to update form_soap_audio ID {$targetFormsId}. SQL Error: " . $GLOBALS['adodb']['db']->ErrorMsg());
                return false;
            }
        } else {
            $insertSql = "INSERT INTO form_soap_audio (
                                id, date, pid, user, groupname, authorized, activity,
                                subjective, objective, assessment, plan, transcript, remarks
                           ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            $bindings = [
            ...
            if ($insertResult === false || $affectedRowsInsert < 1) {
                error_log("OpenEMRAudioSoapNoteManager CRITICAL: Failed to insert into form_soap_audio with ID {$targetFormsId}. SQL Error: " . $GLOBALS['adodb']['db']->ErrorMsg() . " Affected Rows: " . $affectedRowsInsert);
                return false;
            }
        }

        $newFormName = "Audio2Note SOAP";
        ...
        if (sqlStatement($updateFormsTableSql, [$newFormName, $targetFormsId, $targetFormsId]) === false) {
            error_log("OpenEMRAudioSoapNoteManager WARNING: Failed to update form_name in 'forms' table for ID {$targetFormsId}.");
        } else {
        }
